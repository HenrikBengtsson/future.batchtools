...
for (cores in 1:min(3L, availableCores("multicore"))) {
  ...
  if (!supportsMulticore()) {
    message(sprintf("batchtools multicore futures are not supporting on '%s'. Falling back to use synchroneous batchtools local futures", .Platform$OS.type))
  }

  for (globals in c(FALSE, TRUE)) {

  message(sprintf("*** batchtools_multicore(..., globals=%s) without globals", globals))

  f <- batchtools_multicore({
    42L
  }, globals=globals)
  stopifnot(inherits(f, "BatchtoolsFuture") || ((cores == 1 || !supportsMulticore()) && inherits(f, "EagerFuture")))
  ...
  message(sprintf("*** batchtools_multicore(..., globals=%s) with globals", globals))
  ## A global variable
  a <- 0
  f <- batchtools_multicore({
    b <- 3
...
##  if ("covr" %in% loadedNamespaces()) v <- 0 else ## WORKAROUND
  ...
  message(sprintf("*** batchtools_multicore(..., globals=%s) with globals and blocking", globals))
  x <- listenv()
  for (ii in 1:4) {
    message(sprintf(" - Creating batchtools_multicore future #%d ...", ii))
...
##  if ("covr" %in% loadedNamespaces()) v <- 1:4 else ## WORKAROUND
  ...
  message(sprintf("*** batchtools_multicore(..., globals=%s) and errors", globals))
  f <- batchtools_multicore({
    stop("Whoops!")
    1
  }, globals=globals)
